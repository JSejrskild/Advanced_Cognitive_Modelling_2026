...
model {
 // rho: weakly centred on equal weighting
  target += beta_lpdf(rho | rho_alpha, rho_beta); // Prior (2,2)
  // kappa: lognormal centered on 2 (SBA equivalent)
  target += lognormal_lpdf(kappa | log(kappa_log_mu), kappa_log_sd); // Prior (log(2), 0.5)
...
generated quantities {
  ...
  real lprior = beta_lpdf(rho | alpha_rho, beta_rho) + lognormal_lpdf(kappa | log(kappa_log_rho), kappa_log_sd);
  
  // Priors
  real rho_prior   = beta_rng(alpha_rho, beta_rho);
  real kappa_prior = lognormal_rng(log(kappa_log_rho), kappa_log_sd);
