...
df = data.frame(RL_choice = out, other_choice = feedback, EV = V, P = state)

data_stan = list(t = n, choice =out, feedback = feedback, initialV = V_0, W = cond_w, initial_state = state_0,
                 alpha_prior_mu = 0, alpha_prior_sd = 1.5)

mod =cmdstan_model('~/Projects/Repositories/Advanced_Cognitive_Modelling_2026/2_matching_pennies_stan_models/src/RL_model.stan')

fit = mod$sample(data = data_stan, chains = 4, parallel_chains = 4, iter_warmup =1000, iter_sampling =2000)

as.numeric(fit$summary("alpha")["mean"])